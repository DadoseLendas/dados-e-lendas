...
export const extractMonsterSheetFromPdf = async (pdfUrl: string, tokenName: string) => {
  ...
  const pdfParse =
    typeof moduleAny.default === "function"
      ? moduleAny.default
      : typeof moduleAny.pdfParse === "function"
        ? moduleAny.pdfParse
  ...
  if (!pdfParse) {
    throw new Error("Biblioteca de PDF indisponível no servidor");
  }

  const response = await fetch(normalizePdfUrl(pdfUrl), { redirect: "follow" });
  ...
  if (!text.trim()) {
    throw new Error("O PDF não retornou texto pesquisável");
  }
