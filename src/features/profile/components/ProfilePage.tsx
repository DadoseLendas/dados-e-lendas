...
export default function PerfilView() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); throw new Error(d.error || 'Erro ao excluir conta'); }
      await supabase.auth.signOut();
      router.push('/login?reason=conta-excluida');
    } catch (err) {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('id, nickname, display_name, avatar_url, bio')
        .eq('id', user.id)
        .single();
  ...
  return (
    <div className="min-h-screen bg-[#050a05] flex flex-col font-sans text-white">
      ...
      <main className="flex-grow w-full max-w-[1000px] mx-auto animate-in fade-in zoom-in duration-300 py-12 px-6">
        ...
        <div className="w-full max-w-[1000px] mx-auto px-6 pb-12">
        <div className="rounded-2xl border border-red-900/40 bg-red-950/10 p-6">
          <h3 className="text-red-400 text-sm font-black uppercase tracking-widest mb-1">Zona de Perigo</h3>
          <p className="text-[#4a5a4a] text-[11px] mb-4">
            A exclusão é permanente e remove todos os seus dados pessoais (LGPD Art. 18, VI): perfil, personagens, histórico de chat e participações em campanhas.
          </p>
          {!showDeleteConfirm ? (
            <button
              onClick={() => setShowDeleteConfirm(true)}
              className="px-4 py-2 rounded-lg border border-red-700/50 text-red-400 text-[11px] font-black uppercase tracking-widest hover:bg-red-900/20 transition-all"
            >
              Excluir Minha Conta
            </button>
          ) : (
            <div className="space-y-3">
              <p className="text-red-300 text-[12px]">
                Digite <strong className="font-black">EXCLUIR</strong> para confirmar:
              </p>
              <input
                value={deleteConfirmText}
                onChange={e => setDeleteConfirmText(e.target.value.toUpperCase())}
                placeholder="EXCLUIR"
                className="w-full bg-black border border-red-700/50 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-red-500"
              />
              <div className="flex gap-3">
                <button
                  onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmText(''); }}
                  className="px-4 py-2 rounded-lg border border-[#1a2a1a] text-[#4a5a4a] text-[11px] font-black uppercase hover:text-white transition-all"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-red-800/50 border border-red-600/50 text-red-200 text-[11px] font-black uppercase hover:bg-red-700/50 transition-all disabled:opacity-40"
                >
