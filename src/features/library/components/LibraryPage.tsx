...
export default function TelaDeLivros() {
  ...
  const handleAddBook = async (e: React.FormEvent) => {
    ...
    // T-15: Validar pdf_url contra SSRF - apenas HTTPS e domínios permitidos
    const pdfUrlTrimmed = newPdfUrl.trim();
    const allowedHosts = ['drive.google.com', 'docs.google.com', 'storage.googleapis.com', 'supabase.co'];
    let pdfUrlValid = false;
    try {
      const parsed = new URL(pdfUrlTrimmed);
      pdfUrlValid = parsed.protocol === 'https:' &&
        allowedHosts.some(h => parsed.hostname === h || parsed.hostname.endsWith('.' + h));
    } catch { pdfUrlValid = false; }
    if (!pdfUrlValid) {
      alert('URL do PDF inválida. Use apenas links de Google Drive, Google Docs ou Supabase (HTTPS).');
      return;
    }

    let finalCoverUrl = newCoverUrl.trim();
    if (!finalCoverUrl) {
      const autoCover = getDriveThumbnail(pdfUrlTrimmed);
