...
export async function uploadCampaignImage(base64Image: string, campaignId: string) {
  ...
  // 0. Verificar autenticação (T-03: era possível fazer upload sem sessão)
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('Não autenticado.');

  // 0.1 Verificar se o usuário é DM da campanha
  ...
  if (!campaign || campaign.dm_id !== user.id) {
    throw new Error('Sem permissão para alterar imagem desta campanha.');
  }
