...
export default function CampanhasPage() {
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      ...
      const { data: ownedCampaigns, error: ownedError } = await supabase
        .from('campaigns')
        .select('id, name, code, created_at, dm_id, image_url')
  ...
  const handleJoinCampaign = async (e: React.FormEvent) => {
    ...
    if (step === 1) {
      // Resolve a campanha pelo código via RPC SECURITY DEFINER.
      // Assim o SELECT de campaigns fica restrito a membro/DM (não vaza códigos),
      // mas ainda é possível entrar informando o código exato.
      const { data: rows, error } = await supabase
        .rpc('find_campaign_by_code', { p_code: joinCode.trim() });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {campaigns.map(campaign => (
                <Card
                  ...
                  onAccess={() => { 
                    setDropdownOpen(null); 
                    window.location.href = `/mesa/${campaign.id}`;
                  }}
