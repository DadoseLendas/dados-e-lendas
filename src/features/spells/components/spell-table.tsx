...
export default function SpellTable({
  ...
  // Sanitização: garante que campaignId seja um UUID válido antes de usar em queries
  const safeCampaignId = useMemo(() => {
  ...
  const loadSpells = useCallback(async () => {
    try {
      ...
      if (safeCampaignId != null) {
        query = query.or(`campaign_id.is.null,campaign_id.eq.${String(safeCampaignId)}`);
      } else {
