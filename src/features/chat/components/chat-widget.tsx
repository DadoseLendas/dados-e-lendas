...
export default function ChatWidget({ campaignId, isDiceReady, onRollDice }: ChatWidgetProps) {
  ...
  // Ref persiste entre re-subscrições — evita duplicatas entre broadcast e postgres_changes
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      if (data) {
        // Busca as 50 mais RECENTES (desc) e inverte para exibir em ordem cronológica.
        // Antes usava ascending+limit → pegava as 50 mais antigas e mensagens novas
        // sumiam ao recarregar depois que a campanha passava de 50 mensagens.
        const ordered = [...data].reverse();
        setMessages(ordered);
        ordered.forEach((m) => receivedIdsRef.current.add(m.id));
        prevMessagesLength.current = ordered.length;
      }
    };
    fetchMessages();
    ...
    chatSub
      .on('broadcast', { event: 'new_message' }, (payload) => {
        ...
        setMessages((prev) => {
          // Guarda extra contra duplicatas (ex.: já inserida localmente)
          if (prev.some((m) => m.id === newMsg.id)) return prev;
          return [...prev, newMsg];
        });
        if (!isOpenRef.current) setUnreadCount((prev) => prev + 1);
      })
      // postgres_changes captura mensagens inseridas sem broadcast:
      // rolagem de dados da ficha, resultados de magia, itens de sistema.
      .on('postgres_changes', {
        ...
        receivedIds.add(newMsg.id);
        setMessages((prev) => {
          // Mensagem já presente (inserida localmente ou via broadcast) → ignora
          if (prev.some((m) => m.id === newMsg.id)) return prev;
  ...
  const handleRollClick = async (diceType: string) => {
    ...
    if (error) console.error('[chat] Falha ao salvar rolagem:', error);
