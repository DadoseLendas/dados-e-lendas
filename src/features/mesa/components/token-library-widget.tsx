...
export default function TokenLibraryWidget({ isOpen, onToggle, onAddToken, onUpload, floating = false }: TokenLibraryWidgetProps) {
  ...
  const handleClose = () => {
    setSelectedCategory(null);
    setSearch('');
    if (!floating) onToggle();
  };

  const header = (
    <div data-drag-handle className="bg-[#0a120a] border-b border-[#1a2a1a] px-4 py-3 flex items-center justify-between cursor-grab active:cursor-grabbing select-none shrink-0">
      ...
      <button onClick={(e) => { e.stopPropagation(); handleClose(); }} className="text-[#4a5a4a] hover:text-white">
        <X size={16} />
      </button>
    </div>
  );
  ...
  const renderContent = (fullHeight = true) => (
    <div className={`bg-[#050a05] border border-[#1a2a1a] rounded-xl shadow-[0_0_40px_rgba(0,0,0,0.9)] overflow-hidden ${fullHeight ? 'flex flex-col max-h-[80vh] w-64' : ''}`}>
      ...
      {/* Pastas */}
      {!selectedCategory && (
        <div className={`${fullHeight ? 'overflow-y-auto flex-1' : ''} p-2 space-y-1`}>
              {categories.length === 0 ? (
