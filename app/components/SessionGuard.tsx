...
export default function SessionGuard() {
  ...
  const handleLogout = useCallback(async (reason: string) => {
    ...
    // 2. Limpa o heartbeat (sessionStorage é limpo automaticamente ao fechar a aba)
    sessionStorage.removeItem('dl_session_heartbeat');
  ...
  useEffect(() => {
    ...
    const checkSessionAndHeartbeat = async () => {
      ...
      const lastHeartbeat = sessionStorage.getItem('dl_session_heartbeat');
      ...
      interval = setInterval(async () => {
        ...
        sessionStorage.setItem('dl_session_heartbeat', Date.now().toString());
      }, 5000);
