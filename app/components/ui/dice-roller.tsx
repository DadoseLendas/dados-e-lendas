    ...
    const init = async () => {
      try {
        ...
        onReady(async (diceType: string, isSecret: boolean) => {
          ...
          // Rolador vê animação livre — valor vem da física real
          const physics: RollPhysics = {
            value:      0, // ignorado para o rolador
            spinForce:  3 + Math.random() * 6,
            throwForce: 3 + Math.random() * 5,
          };

          // Rola livremente sem forçar valor
          ...
          const themeColor = isSecret ? '#ef4444' : (DICE_COLORS[diceType] ?? '#00ff66');
          const result = await diceBoxRef.current.roll([{ qty: 1, sides, themeColor }]);
          const realValue = result[0].value as number;

          clearTimerRef.current = setTimeout(() => diceBoxRef.current?.clear(), 4000);

          // Rolagem secreta do mestre: não envia ao canal
          if (isSecret && isDMRef.current) return realValue;

          // Envia o valor real para os receptores — eles verão o dado cair nesse número
          channel.send({
            type: 'broadcast',
            event: 'roll',
            payload: {
              ...
              physics: { ...physics, value: realValue },
            },
          });
